...
function add_map_content(Rooms){
    /*Constraints
        ...
        -1/3 of all rooms must be monsters   
        -1/4 of all rooms must be treasures     
    */
    ...
    for (let i = 1; i < Rooms.length-1; i++){
        if (Rooms[i].content == 'n'){
            prob = Math.random();
            if (prob < 0.45){
                continue; // 45% chance empty room
            } else if (prob >= 0.45 && prob < 0.72){
                Rooms[i].setContent('M'); //27% chance monster
            } else {
                Rooms[i].setContent('T'); //28% chance treaure
            }
        }
    }
}
...
function printMap() { //generated by ChatGPT-4o
...
function renderMap(){
    ...
    const coordMap = {};
    Rooms.forEach(r=>{ coordMap[`${r.coord[0]},${r.coord[1]}`]=r; });
    ...
    Rooms.forEach(r=>{
        const [x,y]=r.coord;
        const gx=(x-minX)*2;
        const gy=(maxY-y)*2; // invert Y for visual top‑down
        const cls=`room-${r.content}`;
        grid[gy][gx]=`<span class="${cls}">${r.content}</span>`;
        if(r.N>=0) grid[gy-1][gx]='|';
        if(r.S>=0) grid[gy+1][gx]='|';
        if(r.E>=0) grid[gy][gx+1]='-';
        if(r.W>=0) grid[gy][gx-1]='-';
    });

    // Convert grid to HTML string
    const htmlRows = grid.map(row=>row.join('')).join('\n');
    mapEl.innerHTML = htmlRows;
}

// ---------- UI ----------
document.getElementById('generateBtn').addEventListener('click',()=>{
    const count = parseInt(document.getElementById('roomCount').value,10)||18;
    generate_dungeon(count);
