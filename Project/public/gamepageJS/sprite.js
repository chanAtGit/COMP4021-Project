...
const Sprite = function(ctx, x, y) {
    ...
    // This is the rotated angle (radians) of the sprite. For player sprite to turn towards the cursor.
    let rotation = 0;
    ...
    // This function sets the rotation of the sprite.
    // - `value` - The new rotation angle
    const setRotation = function(value) {
        rotation = value;
        return this;
    };
    ...
    const drawSprite = function() {
        ...
        /* Translate to the sprite's center position */
        ctx.translate(x, y);

        /* Rotate the canvas context */
        ctx.rotate(rotation);

        /* Draw the sprite centered at (0, 0) relative to the translated position */
        ctx.imageSmoothingEnabled = false; //set imageSmoothing to false. no blurring
        ctx.drawImage(sheet, 
            sequence.x + index * sequence.width, sequence.y, 
            sequence.width, sequence.height, //parameters for the sprite in the sprite sheet
            -size.width / 2, -size.height / 2, //center the sprite at (0, 0)
            size.width, size.height //parameters for the image in the canvas area
        );
    ...
    const update = function(time) {
        ...
        if (time - lastUpdate >= sequence.timing){ //enough time has passed since last update
            if (sequence.loop){ //if seqence is looping
                index++; 
                index = index%sequence.count;
            } else { //if seqence is not looping
                if (index < sequence.count - 1){
                    index++;
                }
            }
            lastUpdate = time; //update lastUpdate
        }
